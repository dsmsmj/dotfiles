...
Comprehensive performance optimization guide for React applications, designed for AI agents and LLMs. Applicable to any React project regardless of framework (CRA, Vite, Remix, Next.js, etc.). Contains 40+ rules across 8 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
**Alternative (bundler configuration):**

Most modern bundlers support auto-optimizing barrel imports:

```js
// vite.config.js
export default {
  optimizeDeps: {
    include: ['lucide-react', '@mui/material']
  }
}

// next.config.js (Next.js 13.5+)
module.exports = {
  experimental: {
...
### 3.5 Non-Blocking Background Tasks After Response
...
**Correct: non-blocking with queueMicrotask**
